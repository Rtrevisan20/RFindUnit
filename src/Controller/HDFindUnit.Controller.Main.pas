unit HDFindUnit.Controller.Main;

interface

uses
  ToolsAPI,
  HDFindUnit.Controller.CompilerInterceptor,
  HDFindUnit.Controller.EnvironmentController,
  HDFindUnit.View.FormMessage,
  HDFindUnit.Model.Header,
  HDFindUnit.View.PaintUnusedUses,
...
type
  ...
  private
    ...
    procedure CodeInsightWithImport(const Context: IOTAKeyContext; KeyCode: TShortCut; var BindingResult: TKeyBindingResult);
...
begin
  ...
  NewItem.Caption := Translation.GetMenuForceRegister;
...
begin
  ...
  vBindingServices.AddKeyBinding([ShortCut(VK_SPACE, [ssCtrl])], CodeInsightWithImport, nil);
end;
...
procedure TRFindUnitMain.CodeInsightWithImport(
    const Context: IOTAKeyContext;
    KeyCode: TShortCut;
    var BindingResult: TKeyBindingResult
);
...
begin
  ...
  begin
    ...
    try
      ...
      TThread.Synchronize(nil, procedure
      begin
        TfrmMessage.ShowInfoToUser(ResultMsg);
      end);
    except
      on E: Exception do
      begin
        Logger.Error('GetUnusedUses error: %s', [E.Message]);
        try
          TThread.Synchronize(nil, procedure
          begin
            TfrmMessage.ShowInfoToUser(Translation.GetUnusedUsesError(E.Message));
          end);
        except
        end;
      end;
...
begin
  ...
  TfrmMessage.ShowInfoToUser(Translation.GetOrganizeUsesDone + ' ' + Translation.GetOrganizeUsesWarning);
end;
